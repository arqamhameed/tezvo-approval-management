...
class SaleOrderApproval(models.Model):
    ...
    def action_submit_for_approval(self):
        ...
        if not self.id:
            raise UserError('Please save the Sales Order before submitting for approval.')
        if not self._approval_required_for_document():
            raise UserError('No approval rule is configured for this Sales Order.')
        if self.approval_request_id and self.approval_request_id.state in ('draft', 'waiting', 'partial'):
            raise UserError('An active approval request already exists for this Sales Order.')
    ...
    def action_cancel(self):
        for order in self:
            if (
                ...
                if document_type and not document_type.allow_cancel_approved_document:
                    raise UserError(
                        'This Sales Order has already been approved and cannot be cancelled '
        ...
        for order in self:
            if order.state == 'cancel' and order.approval_request_id:
                order._approval_cancel_linked_request('Sales Order was cancelled.')
                order.write({
    ...
    def action_draft(self):
        ...
        for order in self:
            if order.approval_request_id and order.approval_request_id.state not in ['cancelled', 'rejected', 'approved']:
                order._approval_cancel_linked_request('Sales Order was reset to draft.')
            order.write({
                'approval_state': 'draft',
                'approval_request_id': False,
                'current_approval_level_id': False,
    ...
    def _approval_create_request(self):
        ...
        approval_request = self.env['approval.request'].with_context(
            ...
            'name': f'Sales Order Approval: {self.name or "/"}',
