...
class MrpUnbuildApproval(models.Model):
    _name = 'mrp.unbuild'
    _inherit = ['mrp.unbuild', 'approval.mixin']
    ...
    def _compute_approval_required(self):
        for unbuild in self:
            unbuild.approval_required = unbuild._approval_required_for_document()
    ...
    def action_submit_for_approval(self):
        self.ensure_one()
        if self.state != 'draft':
            raise UserError('Only draft Unbuild Orders can be submitted for approval.')
        if not self.id:
            raise UserError('Please save the Unbuild Order before submitting for approval.')
        if not self._approval_required_for_document():
            raise UserError('No approval rule is configured for this Unbuild Order.')
        if self.approval_request_id and self.approval_request_id.state in ('draft', 'waiting', 'partial'):
            raise UserError('An active approval request already exists for this Unbuild Order.')
    ...
    def _approval_create_request(self):
        ...
        approval_request = self.env['approval.request'].with_context(
            ...
            'name': f'Unbuild Order Approval: {self.name or "/"}',
